...
**********************************************************************************************/
    ...
    function zl_projectItemLabels (thisObj) {
        ...
        var userLabels = getUserLabels(defaultLabels);
        var curColour = 0;
        var colourItems = {
            All: true,
            Folder: false,
        ...
        function createPalette (thisObj) {
            ...
            { // Dropdown
                ...
                var colourPanel = win.add('panel', undefined, 'Colour');
                    var colourList = colourPanel.add('dropdownlist');

                        populateDDL(colourList);
                        colourList.selection = 0;

                        colourList.onChange = function() {
                            curColour = colourList.selection.index;
                        };
            } // end Dropdown
        ...
        // Helpers for building ddl
        function populateDDL (ddl) {
            for (var i = 0, il = userLabels.length; i < il; i++)
                makeNewListItem(i, ddl);
        }

        function makeNewListItem(listIdx, list) {
            list.add("item", userLabels[listIdx].name, list.items.length);
            var newListItem = list.items[list.items.length - 1];
            initListItem(newListItem, listIdx);
        }

        function initListItem(item, labelIdx) {
            item.labelIndex = labelIdx;
            item.text = userLabels[labelIdx].name;
            item.image = buildPngSwatch(userLabels[labelIdx].value);
        }

        function buildPngSwatch(valueString) {
            return pngSwatch(hexToRGB(colorToHEX(valueString)));
        }
        ...
        // Helpers for Custom Label Colours
        function getGeneralPrefsFileAsString() {
            ...
            var folderPath = String(Folder.userData) + "/Adobe/After Effects/" + appVersion;

            if (system.osName !== "MacOS") {
                // this is Windows
                if (appVersion < 12.0)
                    prefsSuffix = "-x64 Prefs.txt";
            } else {
                // this is Mac
                if (appVersion < 12.0)
                    prefsSuffix = "-x64 Prefs";
                folderPath = String(Folder.userData.parent) + "/Preferences/Adobe/After Effects/" + appVersion;
            }

            var filePath = "Adobe After Effects " + appVersion + prefsSuffix;
        ...
        function updateLabelValues(updatedNamesArray) {
            ...
            var labelColoursStringBlock = prefsAsString.substring(labelBlockStartEnd.start, labelBlockStartEnd.end);
            var labelColoursArray = labelColoursStringBlock.split("\n");
            labelColoursArray.shift();

            var encodedObjectArray = buildEncodedObjectArray(labelColoursArray);

            for (var i = 0, il = encodedObjectArray.length; i < il; i++) {
                var label = encodedObjectArray[i];
                updatedNamesArray[label.labelID].value = label.labelString;
            }

            return updatedNamesArray;
        }

        function updateLabelNames(labelArray) {
            var appVersion = parseFloat(app.version);
            var updatedLabelArray = labelArray;

            for (var i = 1; i <= 16; i++){
                if (appVersion < 12.0) {
                    if (app.preferences.havePref("Label Preference Text Section 5", "Label Text ID 2 # " + i))
                        updatedLabelArray[i].name = app.preferences.getPrefAsString("Label Preference Text Section 5", "Label Text ID 2 # " + i);
                } else {
                    if (app.preferences.havePref("Label Preference Text Section 5", "Label Text ID 2 # " + i, PREFType.PREF_Type_MACHINE_INDEPENDENT))
        ...
        function getUserLabels(labelArray) {
            var updatedNamesArray = updateLabelNames(labelArray);
            var updatedValuesArray = updateLabelValues(updatedNamesArray);
        ...
        function findLabelBlockStartEnd(prefsAsString) {
            return {
                ...
                end: prefsAsString.indexOf('["Label Preference Indices Section 5"]', this.start)-2 //-2 because we're 2 lines down
            };
        }

        function buildEncodedObjectArray(labelColoursArray) {
            var labelColoursObjectArray = [];
            for (var i = 0, il = labelColoursArray.length; i < il; i++) {
                var thisLabel = labelColoursArray[i];

                var thisLabelObject = {
                    labelID: thisLabel.split(" = ")[0],
                    labelString: thisLabel.split(" = ")[1]
                };

                var labelID = thisLabelObject.labelID;
                var id = labelID.replace("Label Color ID 2 # ","").replace("\"","");
                thisLabelObject.labelID = parseInt(id.slice(0, -1));

                labelColoursObjectArray.push(thisLabelObject);
            }
            return labelColoursObjectArray;
        }
